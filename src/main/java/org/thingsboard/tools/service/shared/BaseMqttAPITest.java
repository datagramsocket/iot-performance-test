    ...
    private void sendAndWaitPack(List<DeviceClient> pack, AtomicInteger totalWarmedUpCount) throws InterruptedException {
        ...
        for (DeviceClient deviceClient : pack) {
            restClientService.getScheduler().submit(() -> {
                deviceClient.getMqttClient().publish(getWarmUpTopic(), Unpooled.wrappedBuffer(getData(deviceClient.getDeviceName())), MqttQoS.valueOf(mqttQoS))
                        .addListener(future -> {
                                    if (future.isSuccess()) {
...
//            client.disconnect();
//            throw new RuntimeException(String.format("Failed to connect to MQTT broker at %s:%d, %s.", mqttHost, mqttPort, ex.toString()));
            log.warn("Failed to connect to MQTT broker at {}:{}, {}.", mqttHost, mqttPort, ex.toString());
        }
        if (!result.isSuccess()) {
//            connectFuture.cancel(true);
//            client.disconnect();
//            throw new RuntimeException(String.format("Failed to connect to MQTT broker at %s:%d. Result code is: %s", mqttHost, mqttPort, result.getReturnCode()));
            log.warn("Failed to connect to MQTT broker at {}:{}. Result code is: {}", mqttHost, mqttPort, result.getReturnCode());
        }
        return client;
    }
    ...
    protected void reportMqttClientsStats() {
        for (MqttClient mqttClient : mqttClients) {
            mqttClient.publish("v1/devices/me/telemetry", Unpooled.wrappedBuffer("{\"msgCount\":0}".getBytes(StandardCharsets.UTF_8)), MqttQoS.valueOf(mqttQoS)).addListener(future -> {
                        if (future.isSuccess()) {
    ...
    protected void runApiTestIteration(int iteration,
        ...
        try {
            ...
            for (int i = 0; i < testMessagesPerSecond; i++) {
                ...
                restClientService.getWorkers().submit(() -> {
                    client.getMqttClient().publish(getTestTopic(), Unpooled.wrappedBuffer(message.getData()), MqttQoS.valueOf(mqttQoS))
                            .addListener(future -> {
                                        if (future.isSuccess()) {
            ...
            log.info("[{}] Time spent: {} milliseconds, completed performance iteration. Success: {}, Failed: {}, Alarms: {}", iteration, timeStop - timeStart, successPublishedCount.get(), failedPublishedCount.get(), alarmCount);
